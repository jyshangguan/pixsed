...
def centroid_SNR(map_flux, map_flux_err, ref_band, rows, cols, method='flux_max',mask=None):
    if method =='SNR_max':
        idx = np.unravel_index(np.argmax(map_flux[ref_band][rows,cols]/map_flux_err[ref_band][rows,cols]), 
                        map_flux[ref_band][rows,cols].shape)
        bin_y_cent, bin_x_cent = rows[idx[0]], cols[idx[0]]
    elif method =='flux_max':
        idx = np.unravel_index(map_flux[ref_band][rows,cols].argmax(), map_flux[ref_band][rows,cols].shape)
        bin_y_cent, bin_x_cent = rows[idx[0]], cols[idx[0]]
    else:
        raise ValueError('Centroid type not recognized, please choose from: SNR_max, or flux_max!')
    return bin_y_cent, bin_x_cent

def increase_rad_center(bin_rad,bin_x_cent,bin_y_cent,dim_x,dim_y,gal_region,pixbin_map,map_flux_trans,map_flux_err_trans,SN_threshold):
    del_dim = bin_rad+3 
    ...
    return rows1,cols1,tot_bin_flux,tot_bin_flux_err2,tot_SNR, idx0

def increase_rad(rmin,rmax,del_dim,bin_x_cent,bin_y_cent,dim_x,dim_y,gal_region,pixbin_map,map_flux_trans,map_flux_err_trans,SN_threshold):
    xmin = int(bin_x_cent-del_dim)
    xmax = int(bin_x_cent+del_dim)
    ymin = int(bin_y_cent-del_dim)
    ymax = int(bin_y_cent+del_dim)
...
def calc_pix_chi2(xc,yc,map_flux_trans,map_flux_err_trans,factor):
...
def calc_chi2_center_edge(dim_x, dim_y,nbands,rows1,cols1,bin_x_cent,bin_y_cent,map_flux_corr_trans,map_flux_err_corr_trans):
...
def expand_pix(count_bin,pixbin_map,rows1,cols1,map_bin_flux,map_bin_flux_err,tot_bin_flux,tot_bin_flux_err2,cumul_npixs_in_bin):
    ...
    return count_bin,pixbin_map,map_bin_flux,map_bin_flux_err,cumul_npixs_in_bin

def pixel_binning_images(images, var_images, gal_region, ref_band=None, Dmin_bin=2.0, SNR=None, redc_chi2_limit=4.0, del_r=2.0):
    """Function for pixel binning on multiband image.  
    This method is enherited from piXedfit.
    https://pixedfit.readthedocs.io
    https://ui.adsabs.harvard.edu/abs/2017MNRAS.469.2806A/abstract
    https://ui.adsabs.harvard.edu/abs/2021ApJS..254...15A/abstract

    :param images:
        Input science images in a list format. 
    ...
    # get image size
    dim_y = images.shape[1]
    dim_x = images.shape[2]

    map_flux = images
    map_flux_err = var_images
    ...
    # modify negative fluxes in a given band with the minimum flux, only used in calculating chi-square for the evaluation of the SED shape similarity  
    ...
    yc,xc = centroid_SNR(map_flux, map_flux_err, ref_band, rows, cols)
    
    status_add = 1
    factor = 0.01
    while status_add == 1:
        pix_chi2 = calc_pix_chi2(xc,yc,map_flux_trans,map_flux_err_trans,factor)
    ...
    iter = 0
    while len(rows)>0:
        bin_y_cent, bin_x_cent = centroid_SNR(map_flux, map_flux_err, ref_band, rows, cols)
        #=> first circle
        bin_rad = 0.5*Dmin_bin
        rows1,cols1,tot_bin_flux,tot_bin_flux_err2,tot_SNR, idx0 = increase_rad_center(bin_rad,
                            bin_x_cent,bin_y_cent,dim_x,dim_y,gal_region,pixbin_map,map_flux_trans,map_flux_err_trans,SN_threshold)
        iter += 1

        if len(idx0[0]) == nbands:
            count_bin,pixbin_map,map_bin_flux,map_bin_flux_err,cumul_npixs_in_bin = expand_pix(count_bin,
                    pixbin_map,rows1,cols1,map_bin_flux,map_bin_flux_err,tot_bin_flux,tot_bin_flux_err2,cumul_npixs_in_bin)
        ...
        #=> increase radius of the circle
            ...
            while stat_increase==1:
                ...
                rmax = rmin + del_r
                del_dim = rmax + 3
                rows1,cols1 = increase_rad(rmin,rmax,del_dim,
                        bin_x_cent,bin_y_cent,dim_x,dim_y,gal_region,pixbin_map,map_flux_trans,map_flux_err_trans,SN_threshold)
                # check similarity of SED shape
                pix_chi2 = calc_chi2_center_edge(dim_x, dim_y,nbands,rows1,cols1,bin_x_cent,bin_y_cent,map_flux_corr_trans,map_flux_err_corr_trans)
                ...
                if len(idx0[0]) == nbands:
                    count_bin,pixbin_map,map_bin_flux,map_bin_flux_err,cumul_npixs_in_bin = expand_pix(count_bin,
                            pixbin_map,cumul_rows,cumul_cols,map_bin_flux,map_bin_flux_err,tot_bin_flux,tot_bin_flux_err2,cumul_npixs_in_bin)
                    stat_increase = 0
                    ...
                    if len(tidx[0]) < nbands:
                        ...
                        count_bin,pixbin_map,map_bin_flux,map_bin_flux_err,cumul_npixs_in_bin = expand_pix(count_bin,
                            pixbin_map,rows_rest,cols_rest,map_bin_flux,map_bin_flux_err,tflux,tflux_err2,cumul_npixs_in_bin)
                        stat_increase = 0
                        break
    ...
    return pixbin_map, map_bin_flux_trans, map_bin_flux_err_trans

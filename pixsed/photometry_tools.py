...
class Image(object):
    ...
    def __init__(self, filename=None, data=None, header=None, coord_sky=None,
                 ...
                 temp_path='temp', temp_name=None, clean_temp_path=False, 
                 verbose=True):
        """
        Parameters
        ----------
    ...
    def background_properties(self, mask=None, sigma=3, maxiters=5, f_sample=1, **kwargs):
        '''
        ...
        **kwargs : Other parameters of sigma_clipped_stats()
        ...
        if mask_coverage is not None:
            mList.append(mask_coverage)
        
        if mask_background is not None:
            mList.append(mask_background)
    ...
    def background_properties_deprecated(self, mask_type='quick', sigma=3, maxiters=5, f_sample=1, **kwargs):
        '''
        Calculate the mean, median, and std of the background using sigma clip.
    ...
    def background_subtract(self, background_type='median', plot=False, norm_kwargs=None):
        ...
        self.dump_temp_image(data_subbkg, name='data')
    ...
    def create_temp_dir(self):
        ...
        '''
        if self._temp_path == 'temp':
            if not os.path.exists('./temp'):
                os.makedirs('./temp')
        
            tempfile.tempdir = './temp'
            self._temp_dir = tempfile.mkdtemp(prefix='pixsed_image')
        elif self._temp_path is None: # Use the current path if None
            self._temp_dir = './'
        else:
            if not os.path.exists(self._temp_path):
                os.makedirs(self._temp_path)
            self._temp_dir = self._temp_path

        if self._temp_name is None:
            #self._temp_name = f'pixsed_image_{uuid.uuid4().hex}'
            self._temp_name = f'pixsed_image'
        else:
            self._temp_name = f'{self._temp_name}'
    ...
    def detect_source_extended(self, threshold_o:float, threshold_i:float, 
        ...
        if image is not None:
            ...
            image = self.fetch_temp_image('data')
            image_name = 'Background subtracted data'
        assert image is not None, 'Cannot find data_convolved nor data!'
    ...
    def gen_mask_background(self, threshold, npixels=12, mask=None, connectivity=8,
        ...
        if mask_coverage is not None:
            ...
            if mask is None:
                ...
                #mask |= mask_coverage
                mask = np.logical_or(mask, mask_coverage)
    ...
    def gen_mask_contaminant(self, mask=None, expand_inner=1, expand_edge=1.2,
        ...
        # Include the added mask if it is not None
        if mask is not None:
            mList.append(mask)
    ...
    def gen_psf_model(self, extract_size=25, xmatch_radius=3, plx_snr=3,
        ...
        '''
        data = self.fetch_temp_image('data')
        #data_subbkg = self.fetch_temp_image('data')
        segment_outer = self.fetch_temp_segm('segment_outer')
    ...
    def gen_segment_inner(self, detect_thres=5., psf_fwhm=None, 
        ...
        sources = daofind(data, mask=~mask_galaxy_inner)

        segm_data = np.zeros_like(data, dtype=int)
    ...
    def gen_target_mask(self, threshold, npixels=12, mask=None, connectivity=8,
        ...
        data = self.fetch_temp_image('data')
        assert data is not None, err_text
        ...
        mask, smap, cdata = gen_image_mask(data, threshold,
    ...
    def gen_data_variance(self, filename=None, extension=0, f_flux=0, box_size=None, filter_size=5):
        ...
        if filename is not None:
            ...
            data = self.fetch_temp_image('data')
            assert data is not None, 'Cannot find the data_subbkg!'

            data_variance = self.gen_variance_background(box_size=box_size, filter_size=filter_size)
            data_variance[mask_galaxy] = (data_variance + np.square(f_flux * data))[mask_galaxy]
    ...
    def plot_summary(self, fig=None, axs=None, norm_kwargs=None,
        ...
        if data is not None:
            ...
            ax.text(0.5, 0.5, 'No data', fontsize=24, transform=ax.transAxes,
                    ha='center', va='center')
        ax.set_title('(a) Image', fontsize=18)
        ...
        if model_galaxy is not None:
            ...
            ax.text(0.5, 0.5, 'No data', fontsize=24, transform=ax.transAxes,
                    ha='center', va='center')
        ax.set_title('(b) Galaxy model', fontsize=18)
        ...
        if mask_background is not None:
            ...
            ax.text(0.5, 0.5, 'No data', fontsize=24, transform=ax.transAxes,
                    ha='center', va='center')
        ax.set_title('(c) Background mask', fontsize=18)
        ...
        ax.set_title('(d) Contaminant mask', fontsize=18)
        ...
        ax.set_title('(e) Combined segmentation', fontsize=18)
...
class Atlas(object):
    ...
    def gen_mask_manual(self, image_index:int, verbose=True):
        ...
        # Start to work
        data = img_ref.fetch_temp_image('data')
    ...
    def plot_single_image(self, image_index:int, fig=None, axs=None,
        ...
        data_subbkg = img.fetch_temp_image('data')
